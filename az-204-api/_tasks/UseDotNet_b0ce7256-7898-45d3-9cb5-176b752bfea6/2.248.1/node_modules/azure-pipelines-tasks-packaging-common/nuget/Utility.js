...
exports.getSourcesFromNuGetConfig = exports.getNuGetFeedRegistryUrl = exports.setConsoleCodePage = exports.locateCredentialProvider = exports.resolveToolPath = exports.stripLeadingAndTrailingQuotes = exports.resolveWildcardPath = exports.resolveFilterSpec = exports.getPatternsArrayFromInput = void 0;
const fs = require("fs");
const ltx = require("ltx");
const path = require("path");
const tl = require("azure-pipelines-task-lib/task");
const locationUtilities = require("../locationUtilities");
...
exports.getPatternsArrayFromInput = getPatternsArrayFromInput;
// Attempts to resolve paths the same way the legacy PowerShell's Find-Files worked
function resolveFilterSpec(filterSpec, basePath, allowEmptyMatch, includeFolders) {
...
exports.resolveFilterSpec = resolveFilterSpec;
function resolveWildcardPath(pattern, allowEmptyWildcardMatch, includeFolders) {
    ...
    if (!pattern) {
        ...
        const patternFilter = tl.filter(pattern, {
            matchBase: true,
            nobrace: true,
            noext: true,
            nocomment: true,
...
exports.resolveWildcardPath = resolveWildcardPath;
function stripLeadingAndTrailingQuotes(path) {
    if (path.length === 0) {
        return path;
    }
    let left = 0;
...
exports.stripLeadingAndTrailingQuotes = stripLeadingAndTrailingQuotes;
function resolveToolPath(path) {
    return tl.resolve(path);
}
exports.resolveToolPath = resolveToolPath;
function locateCredentialProvider(useV2CredProvider) {
...
exports.locateCredentialProvider = locateCredentialProvider;
...
exports.setConsoleCodePage = setConsoleCodePage;
function getNuGetFeedRegistryUrl(packagingCollectionUrl, feedId, project, nuGetVersion, accessToken, useSession) {
    return __awaiter(this, void 0, void 0, function* () {
        // If no version is received, V3 is assumed
        const registryType = nuGetVersion && ProductVersionHelper_1.getVersionFallback(nuGetVersion).a < 3
        ...
        return yield locationUtilities.getFeedRegistryUrl(packagingCollectionUrl, registryType, feedId, project, accessToken, useSession);
    });
}
exports.getNuGetFeedRegistryUrl = getNuGetFeedRegistryUrl;
function getSourcesFromNuGetConfig(configPath) {
    ...
    if (!xml.nameEquals(new ltx.Element("configuration"))) {
        if (xml.nameEquals(new ltx.Element("packages"))) {
            throw new Error(tl.loc("NGCommon_NuGetConfigIsPackagesConfig", configPath, tl.getVariable("Task.DisplayName")));
        }
        else {
            throw new Error(tl.loc("NGCommon_NuGetConfigIsInvalid", configPath));
        }
...
exports.getSourcesFromNuGetConfig = getSourcesFromNuGetConfig;
